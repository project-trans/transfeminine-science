...
在治疗的第 1 年内出现该事件的风险高于随后治疗的几年（请参阅[注意事项]({{< ref "#precautions" >}})）。
...
该相对风险不依赖于年龄或用药持续时间。
然而，因为基线风险高度依赖年龄，使用 HRT 的女性患中风的总体风险随年龄的增长而增加（请参阅[注意事项]({{< ref "#precautions" >}})）。
...
根据该雌激素/孕激素联合制剂中活性药物成分的性质和数量，它可能导致葡萄糖和血浆胰岛素反应过度（葡萄糖耐量下降）。
...
下面所列的任何一种情况存在时，不应开始激素补充治疗（HRT）。
如果在 HRT 用药过程中出现下列任何一种情况，应立即停药。
